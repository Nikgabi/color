...
function test_input($data) {
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
		...
		$stmt = $con->prepare("SELECT AVG(NULLIF($metabl, '')) as avg_value FROM aimo_data ");
		$stmt->execute();
		...
		if (isset($_SESSION['id_user']) && $_SESSION['role'] == 'visitor') {
			...
			$stmt1 = $con->prepare("SELECT AVG(NULLIF($metabl, '')) as avg_value FROM aimo_data WHERE user_id=? ");
			$stmt1->bind_param("i", $user_id);
			$stmt1->execute();
			$result1 = $stmt1->get_result()->fetch_assoc();
			$avg_user = $result1['avg_value'];
			
			$stmt2 = $con->prepare("SELECT NULLIF($metabl, '') as metabliti , DATE(created_at) as date FROM aimo_data WHERE user_id=?");
			...
			if (!empty($patient_ids)) {
				...
				$query2 = "SELECT AVG(NULLIF($metabl, '')) as avg_value FROM aimo_data WHERE user_id IN ($placeholders) ";
...
<head>
	<title> Διαγράμματα αιματολογικών εξετάσεων</title>
  ...
  <section class="about_section">
    <div class="container">
      <div class="row">
		...
		<h2 style="font-weight: bold; text-align: center;">Διαγράμματα αιματολογικών εξετάσεων</h2>
    <form name="loipes" action="" method="POST"  style="display: flex; flex-direction: column; align-items: center; gap: 15px;">
        ...
        <div style="display: flex; flex-direction: row; align-items: center; gap: 10px; width: 100%; max-width: 500px;">
            ...
            <select name="er1" required style="width: 100%; padding: 8px; height: 200px; overflow: auto;" >
                ...
                <option value="lemfo">Λεμφοκύτταρα</option>               
                <option value="mono">Μονοκύτταρα</option>
                <option value="ios">Ηωσινόφυλλα</option>
                <option value="bas">Βασεόφυλλα</option>
				<option value="plt">Αιμοπετάλια</option>
				<option value="Fe">Σίδηρος</option>  
                <option value="ferit">Φεριτίνη</option>
                <option value="b12">Βιταμίνη Β12</option>
                <option value="filiko">Φυλικό οξύ</option>
                <option value="pt">Χρόνος προθρομβίνης</option>
                <option value="pt1">Χρόνος μάρτυρα</option>               
                <option value="inr">INR</option>
                <option value="ptt">Χρόνος θρομβοπλαστίνης</option>
                <option value="ino">Ινοδωγόνο</option>
            </select>
