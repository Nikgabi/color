...
function test_input($data) {
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
		...
		if (isset($_SESSION['id_user']) && $_SESSION['role'] == 'visitor') {
			...
			$stmt2 = $con->prepare("SELECT scor_depress as metabliti , DATE(date_created_depress) as date FROM depress_data WHERE depress_id=?");
			$stmt2-> bind_param("i", $depress_id);
			$stmt2-> execute();
			$result2 = $stmt2->get_result()->fetch_all(MYSQLI_ASSOC);
			
			//min max 5last
			$stmtMinMax = $con->prepare("SELECT MIN(NULLIF(scor_depress, '')) as min_val, MAX(NULLIF(scor_depress, '')) as max_val 
								 FROM depress_data WHERE depress_id=?");
			$stmtMinMax->bind_param("i", $depress_id);
			$stmtMinMax->execute();
			$minMax = $stmtMinMax->get_result()->fetch_assoc();
			// 2. Λήψη των 5 τελευταίων τιμών για το διάγραμμα
			$stmtLast5 = $con->prepare("SELECT NULLIF(scor_depress, '') as metabliti, DATE_FORMAT(date_created_depress, '%d-%m') as date 
										FROM depress_data WHERE depress_id=? 
										ORDER BY date_created_depress DESC LIMIT 5");
			$stmtLast5->bind_param("i", $depress_id);
			$stmtLast5->execute();
