...
if($row){	
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        <tr>
			...
			while ($data = mysqli_fetch_assoc($result)) {
				...
				echo "<td style='width: 200px;'>
						  <img src='" . BASE_URL . $data['img'] . "' style='width: 75%;'>
					  </td>";
				...
				echo "<td>" . (!empty($data['d_meet']) ? "<a href='" . BASE_URL .  htmlspecialchars($data['d_meet']) . "' target=''><button>Calendar</button></a>" : "-") . "</td>";
		...
		echo "  <div class='w3-card' style='background-color: rgb(240,240,240); text-align:center; width: 70%; margin: auto;'>
			...
			<h3 style='color:green'>Αν θέλεις να αλλάξεις γιατρό <button><a href='" . BASE_URL . "update_doctor.php'>Click εδώ</a></button> </h3></div><br>
